...
func NewState(
	...
	cs.metricsThreshold.timeThreshold = defaultTimeThreshold
...
func (cs *State) updateHeight(height int64) {
	...
	cs.metricsThreshold.IsOutTime = false
	...
	cs.metricsThreshold.oldMetric.cacheMarkVoteExtensionReceived.ifAdd = false
	...
	cs.metricsThreshold.oldMetric.caheOldQuorumPrevoteDelay = make(map[string]float64)
	cs.metricsThreshold.oldMetric.countDuplicateVot = 0
...
func (cs *State) defaultDecideProposal(height int64, round int32) {
	...
	if cs.ValidBlock != nil {
		...
		cs.metricsThreshold.oldMetric.cacheProposalCreateCount.noValidBlocks = true
		cs.metricsThreshold.oldMetric.cacheProposalCreateCount.count++
...
func (cs *State) defaultDoPrevote(height int64, round int32) {
	...
	*/
	...
	cs.metricsThreshold.oldMetric.cacheMarkProposalProcessed = isAppValid
...
func (cs *State) recordMetrics(height int64, block *types.Block) {
	...
	if height > cs.state.InitialHeight {
		...
		for i, val := range cs.LastValidators.Validators {
			...
			mark := MarkLabelVal{false, false, false, 0, nil}
			...
			if bytes.Equal(val.Address, address) {
				...
				mark.label = label
				mark.votingPower = val.VotingPower
				mark.markValidatorPower = true
				if commitSig.BlockIDFlag == types.BlockIDFlagCommit {
					cs.metrics.ValidatorLastSignedHeight.With(label...).Set(float64(height))

					mark.markValidatorLastSignedHeight = true
			...
			cs.metricsThreshold.oldMetric.cacheValidatorPowerLastSignedMiss = append(cs.metricsThreshold.oldMetric.cacheValidatorPowerLastSignedMiss, mark)
		}
	}
...
func (cs *State) addProposalBlockPart(msg *BlockPartMessage, peerID p2p.ID) (added bool, err error) {
	...
	cs.metricsThreshold.oldMetric.cacheNotBlockGossipPartsReceived = false
	if cs.Height != height {
		cs.Logger.Debug("received block part from wrong height", "height", height, "round", round)
		cs.metrics.BlockGossipPartsReceived.With("matches_current", "false").Add(1)
		cs.metricsThreshold.oldMetric.cacheNotBlockGossipPartsReceived = true
		return false, nil
	}
	...
	cs.metricsThreshold.oldMetric.isDuplicateBlockPart = false
	if !added {
		...
		cs.metricsThreshold.oldMetric.isDuplicateBlockPart = true
	}
...
func (cs *State) addVote(vote *types.Vote, peerID p2p.ID) (added bool, err error) {
	...
	if vote.Height < cs.Height || (vote.Height == cs.Height && vote.Round < cs.Round) {
		...
		cs.metricsThreshold.oldMetric.cacheLateVote = append(cs.metricsThreshold.oldMetric.cacheLateVote, n)
	}
	...
	if vote.Height+1 == cs.Height && vote.Type == cmtproto.PrecommitType {
		...
		if !added {
			...
			if err == nil {
				...
				cs.metricsThreshold.oldMetric.countDuplicateVot++
			}
			return added, err
		}
	...
	if extEnabled {
		...
		if vote.Type == cmtproto.PrecommitType && !vote.BlockID.IsZero() &&
			...
			cs.metricsThreshold.oldMetric.cacheMarkVoteExtensionReceived.ifAdd = true
			cs.metricsThreshold.oldMetric.cacheMarkVoteExtensionReceived.errIsNil = append(cs.metricsThreshold.oldMetric.cacheMarkVoteExtensionReceived.errIsNil, err == nil)
			if err != nil {
				return false, err
			}
		}
	...
	if vote.Round == cs.Round {
		...
		cs.metricsThreshold.oldMetric.cacheMarkVoteReceived = append(cs.metricsThreshold.oldMetric.cacheMarkVoteReceived, cacheMarkVoteReceived{n: n, p: p})
	}
...
func (cs *State) calculatePrevoteMessageDelayMetrics() {
	...
	for _, v := range pl {
		...
		if votingPowerSeen >= cs.Validators.TotalVotingPower()*2/3+1 {
			...
			cs.metricsThreshold.oldMetric.caheOldQuorumPrevoteDelay[cs.Validators.GetProposer().Address.String()] = v.Timestamp.Sub(cs.Proposal.Timestamp).Seconds()
	...
	if ps.HasAll() {
		...
		cs.metricsThreshold.oldMetric.cacheFullPrevoteDelay.isHasAll = true
